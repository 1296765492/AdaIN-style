...
if enc:get(#enc).nOutputPlane ~= nil then
    adain = nn.HistogramMatching(enc:get(#enc).nOutputPlane) -- assume the last encoder layer is Conv
else
    adain = nn.HistogramMatching(enc:get(#enc-1).nOutputPlane) -- assume the last encoder layer is ReLU
end
